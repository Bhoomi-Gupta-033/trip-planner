...
export default function TripPage() {
        ...
        const dayImgResults = await Promise.all(
          days.map(async (day, i) => {
            ...
            const query = firstAct
              ? `${res.data.destination} ${typeof firstAct === "object" ? firstAct.name : firstAct}`
              : res.data.destination;
            const url = await getPlaceImage(query);
            return [i, url];
          }),
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    let y = 10;
    doc.text(`Trip to ${trip.destination}`, 10, y);
    y += 10;
    trip.itinerary.days.forEach((day, i) => {
      doc.text(`Day ${i + 1}`, 10, y);
      y += 6;
  ...
  if (!trip || !trip.itinerary)
    return (
      <div className="text-center mt-20 bg-[var(--bg)] min-h-screen text-[var(--text)]">
        <p className="text-xl">Trip not found 😢</p>
        <a href="/my-trips" className="text-emerald-500 underline mt-4 block">
          ← Back to My Trips
        </a>
      </div>
    );
    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="relative h-72 md:h-96 overflow-hidden">
        ...
        <div className="absolute top-4 right-4 flex gap-2">
          <button
            onClick={saveTrip}
            className="px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium shadow-lg transition-colors"
          >
            Save 💾
          </button>
          <button
          ...
          <a
            href="/my-trips"
            className="px-4 py-2 rounded-lg bg-white/20 hover:bg-white/30 text-white text-sm backdrop-blur transition-colors"
          >
            ← Back
          </a>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        {budget && (
          <div
            onClick={() => setShowBudget(true)}
            className="cursor-pointer p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 hover:border-emerald-500 transition-colors flex justify-between items-center"
          >
            <div>
              <p className="text-sm text-gray-500">Estimated Total Budget</p>
              <p className="text-3xl font-bold text-emerald-500 mt-1">
                ₹{budget.total.toLocaleString()}
              </p>
            </div>
